...
function wpsc_have_valid_shipping_zipcode(){
	if(empty($_SESSION['wpsc_zipcode']) || ('Post Code' == $_SESSION['wpsc_zipcode']) && ($_SESSION['wpsc_update_location']))
		return true;
	else
		return false;
...
function wpsc_shipping_country_list( $shippingdetails = false ) {
	...
	if ( ($zipvalue != '') && ($zipvalue != 'Post Code') ) {
		$color = '#000';
	} else {
		$zipvalue = 'Post Code';
		$color = '#999';
	...
	if ( $uses_zipcode == true ) {
		$output .= " <input type='text' style='color:" . $color . ";' onclick='if (this.value==\"Post Code\") {this.value=\"\";this.style.color=\"#000\";}' onblur='if (this.value==\"\") {this.style.color=\"#999\"; this.value=\"Post Code\"; }' value='" . esc_attr( $zipvalue ) . "' size='10' name='zipcode' id='zipcode'>";
	}
	return $output;
}
