...
if (!class_exists('AudioPlayer')) {
    class AudioPlayer {
		...
		function AudioPlayer() {
			...
			$this->pluginPath = WP_CONTENT_DIR . $template_directory_split[1]."/lib/php";

			$this->pluginURL = get_template_directory_uri() . "/lib/php";
		...
		function addConfigureLink($links, $file) {
			...
			if ($file == $this_plugin) {
				$settings_link = '<a href="options-general.php?page=' . $this->optionsPageName . '">' . __( 'Settings' , 'cosmotheme' ) . '</a>';
