...
if(wpsc_cart_item_count() < 1) :
   print "<p class='checkout_empty'>";
   _e('Your shopping cart is empty.', 'wpsc');
   print "<br />";
   _e('Please add items before attempting to check out.', 'wpsc');
   print "</p>";
   return;
endif;
?>
...
<p class="delimiter">&nbsp;</p>
<table class="checkout_cart" cellpadding="0" cellspacing="0">
   <tr class="header">
      <th class="image"><?php _e('Product', 'wpsc'); ?></th>
      <th class="title">&nbsp;</th>
      <th class="quantity"><?php _e('Quantity', 'wpsc'); ?></th>
      <th class="price"><?php _e('Price', 'wpsc'); ?></th>
      <th class="total"><?php _e('Total', 'wpsc'); ?></th>
      <th class="remove">&nbsp;</th>
   </tr>
      ...
      <tr class="product_row product_row_<?php echo wpsc_the_cart_item_key(); ?> <?php echo $alt_class;?>">
         ...
         <td class="wpsc_product_quantity wpsc_product_quantity_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform qty">
               ...
               <p class="button blue"><input type="submit" value="<?php _e('Update', 'wpsc'); ?>" name="submit" /></p>
            </form>
         </td>
         ...
         <td class="wpsc_product_remove wpsc_product_remove_<?php echo wpsc_the_cart_item_key(); ?>">
            <form action="<?php echo get_option('shopping_cart_url'); ?>" method="post" class="adjustform remove">
               ...
               <input type="hidden" name="key" value="<?php echo wpsc_the_cart_item_key(); ?>" />
               <input type="hidden" name="wpsc_update_quantity" value="true" />
               <p class="button"><input type="submit" value="<?php _e('Remove', 'wpsc'); ?>" name="submit" /></p>
            </form>
         </td>
      </tr>
   ...
   <?php //this HTML displays coupons if there are any active coupons to use ?>
    ...
    <tr class="wpsc_total_before_shipping">
        <td colspan="4" style="text-align:right; padding-right:25px;"><?php _e('Total (excluding shipping):','wpsc'); ?></td>
        <td colspan="2" class="wpsc_total_amount_before_shipping"><?php echo wpsc_cart_total_widget(false,false,false);?></td>
    </tr>
  ...
  <?php if(wpsc_uses_shipping()): ?>
   ...
   <div id="wpsc_shipping_container">
		<h3><?php _e('Calculate Shipping Price', 'wpsc'); ?></h3>
   		<p class="delimiter">&nbsp;</p>
        <div class="cosmo-box error large">
               <!-- Shipping Calculator -->
			   <?php if(wpsc_uses_shipping()) : ?>
                  <table class="productcart">
                     <tr class='wpsc_change_country'>
                        <td colspan='5'>
                           <form name='change_country' id='change_country' action='' method='post'>
                              ...
                              <p class="button fr"><input type='submit' name='wpsc_submit_zipcode' value='Calculate' /></p>
                           </form>
                        </td>
                     </tr>
                     ...
                     <?php if (wpsc_have_morethanone_shipping_quote()) :?>
                        <?php while (wpsc_have_shipping_methods()) : wpsc_the_shipping_method(); ?>
                              <?php if (wpsc_have_shipping_quotes()) : // Don't display shipping method if it doesn't have at least one quote ?>
                              <?php while (wpsc_have_shipping_quotes()) : wpsc_the_shipping_quote();  ?>
                                 <tr class='<?php echo wpsc_shipping_quote_html_id(); ?>'>
                                    <td class='wpsc_shipping_quote_name wpsc_shipping_quote_name_<?php echo wpsc_shipping_quote_html_id(); ?>' colspan='4'>
										<label for='<?php echo wpsc_shipping_quote_html_id(); ?>'>
									   		Shipping Price:&nbsp;&nbsp;&nbsp;<?php echo wpsc_shipping_quote_value(); ?>
										</label>
                                    </td>
                                    
                                    <td class='wpsc_shipping_quote_radio wpsc_shipping_quote_radio_<?php echo wpsc_shipping_quote_html_id(); ?>' style='display:none;'>
							 ...
							 <?php else: // No valid shipping quotes ?>
                                <?php if (wpsc_have_valid_shipping_zipcode()) : ?>
                                      <tr class='wpsc_update_location'>
                                         <td colspan='5' class='shipping_error' >
                                            <?php _e('Please provide a Post Code and click Calculate in order to continue.', 'wpsc'); ?>
                                         </td>
                                      </tr>
                                <?php else: ?>
                                   <tr class='wpsc_update_location_error'>
                                      <td colspan='5' class='shipping_error' >
                                         <?php _e('Invalid Postcode. Please check your details and try again.', 'wpsc'); ?>
                                      </td>
                                   </tr>
                                <?php endif; ?>
                            <?php endif; ?>
   ...
   <?php do_action('wpsc_before_form_of_shopping_cart'); ?>
    ...
    <h3><?php _e('Review and purchase','wpsc'); ?></h3>
   	<p class="delimiter">&nbsp;</p>
    <table  class='wpsc_checkout_table table-4 fl'>
        <tr class="total_price">
            <td class='wpsc_totals'>
            <?php _e('Order Subtotal','wpsc'); ?>:
            </td>
            <td class='wpsc_totals'>
        ...
        <tr class='total_price'>
            <td class='wpsc_totals'>
            <b><?php _e('Total', 'wpsc'); ?>:</b>
            </td>
            <td class='wpsc_totals'>
            <b><span id='checkout_total' class="pricedisplay checkout-total"><?php echo wpsc_cart_total(); ?></span></b>
            </td>
        </tr>
    </table>
...
<div class="checkout_form">
   <table class='wpsc_checkout_table table-1 fl'>
      <?php $i = 0;
        ...
        <?php if(wpsc_checkout_form_is_header() == true){
               ...
               <tr class='same_as_shipping_row'>
                  ...
                  elseif(isset($_POST['submit']) && !isset($_POST['shippingSameBilling']))
                   ...
                   ?>
                   	<div class="cosmo-blockquote boxed">
					...
					<span id="shippingsameasbillingmessage"><?php _e('Your order will be shipped to the billing address.', 'wpsc'); ?></span>
                  </td>
               </tr>
               <?php endif;
            ...
            <?php
               ...
               <?php $email_markup =
               "<div class='wpsc_email_address fr'>
      ...
      <div class='wpsc_make_purchase'>
         <span>
            ...
            <?php endif; ?>
               ...
               <p class="button red "><input type='submit' value='<?php _e('Purchase', 'wpsc');?>' name='submit' class='make_purchase wpsc_buy_button' /></p>
         </span>
      </div>
