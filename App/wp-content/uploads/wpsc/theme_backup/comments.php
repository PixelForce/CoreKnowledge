        ...
        ?>
                <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
                    <article id="comment-<?php comment_ID(); ?>" class="comment-body">
                        <div class="cosmo-comment-thumb"><?php $size = image::asize( 'tcomments' ); echo cosmo_avatar( $comment , $size[0] , DEFAULT_AVATAR ); ?></div>
                        <div class="cosmo-comment-leftpointer"></div>
                        <div class="cosmo-comment-quote">
                            <header class="cosmo-comment-textinfo">
                                
                                <span class="time"><?php printf( __( '%1$s&nbsp;&nbsp;%2$s', 'cosmotheme' ), get_comment_date() , get_comment_time() );  ?></span>
...
<?php 
	...
	$fields =  array(
        'author' => '<label for="author">' . __( 'Your name','cosmotheme' ) . '</label><p class="comment-form-author input">' . '<input class="required" id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"  />' .
                    '</p>',
        'email'  => '<label for="email">' . __( 'Your email','cosmotheme' ) . '</label><p class="comment-form-email input"><input  class="required" id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" />' .
                    '</p>',
        'url'    => '<label for="url">' . __( 'Website','cosmotheme' ) . '</label><p class="comment-form-url input"><input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" />' .
                    '</p>',
	);
	...
	$args = array(	
		'title_reply' => __("Leave a reply",'cosmotheme') ,
		'comment_notes_after' =>'',
		'comment_notes_before' =>'<p class="delimiter">&nbsp;</p><p class="comment-notes">' . __( 'Your email address will not be published.' , 'cosmotheme' ) . '</p>',
        'logged_in_as' =>'<p class="delimiter">&nbsp;</p><p class="logged-in-as">' . __( 'Logged in as' ,'cosmotheme' ) . ' <a href="' . home_url('/wp-admin/profile.php') . '">' . get_the_author_meta( 'nickname' , get_current_user_id() ) . '</a>. <a href="' . wp_logout_url( get_permalink( $post -> ID ) ) .'" title="' . __( 'Log out of this account' , 'cosmotheme' ) . '">' . __( 'Log out?' , 'cosmotheme' ) . ' </a></p>',		
